...
- 📂 **Import direct de notes QCMcam** dans une éval Type A/C : bouton 📂 dans le tableur → liste les fichiers `resultats.csv` du dossier mémorisé (triable par date ou par nom), matching élève par prénom (+ premières lettres du nom si ambigu), détection automatique du barème (= nb de questions effectivement posées) avec 3 choix (mettre à jour la mini-note / créer une nouvelle mini-note / garder), détection des absents (Q* toutes vides → code `A`) et de la date dans le nom de fichier (`AAAA-MM-JJ`).
