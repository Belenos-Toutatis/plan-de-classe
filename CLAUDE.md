...
**Classes inconnues créées à l'import** (case « Créer les classes inconnues », défaut coché ; `opts.createClasses`) : la valeur de la colonne Classe qui ne matche ni id ni nom devient une classe via `_createClassBare(id, nom)` — le cas par défaut de `createClass` extrait (salle 4×10 neuve, `setupClassAccessors`), sans `pushUndo` ni `save` (l'import fait les deux). **L'id est compacté par `_impClassIdFromLabel`** (« 3ème C » → `3C`, « 2nde 3 » → `23`, « 1ère S2 » → `1S2` : l'ordinal collé au niveau est retiré), **le libellé complet reste le nom**. Résumé « ✚ Classes à créer : … » sous l'aperçu et ✚ dans la cellule Classe — mais **pas d'avertissement par ligne** (la cellule et le résumé suffisent, le ⚠ est réservé à ce qui demande un regard). Case décochée → repli sur la classe courante avec avertissement, comme avant.
