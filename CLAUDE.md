...
- **`_csvCellGuard(v)`** dans tout échappement de cellule d'export CSV, AVANT le quoting : une cellule commençant par `=` `+` `-` `@` TAB ou CR est interprétée comme FORMULE par Excel/LibreOffice à l'import (le quoting CSV ne protège pas) → préfixée d'une apostrophe. Branché dans les 3 exports CSV (positions, bilan des notes, notes d'une éval).
