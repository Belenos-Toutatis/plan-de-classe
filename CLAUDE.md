...
- **Les deux catalogues sont RÉORDONNABLES**, parce que leur ordre est exactement celui des puces et des boutons proposés ensuite : l'enseignant met en tête ce qu'il pose le plus souvent. **Deux gestes pour le même résultat**, volontairement redondants — les boutons **↑ / ↓** (précis, un pas par clic) et le **glisser** (rapide quand on remonte une ligne de loin).
  ...
  - Glisser par **`_makeSortable(containerId, onMove)`** — Pointer Events, donc souris/doigt/stylet par le même chemin (`_tdAttach` est câblé sur la grille du plan, non réutilisable). Les listes sont reconstruites à chaque render mais leur **conteneur est statique dans le HTML** : les écouteurs s'installent **une seule fois** sur lui (`dataset.sortableInit`), seul le callback est rafraîchi. Les lignes portent `.sort-row` + `data-sort-idx`, la poignée `.sort-handle` (⠿).
