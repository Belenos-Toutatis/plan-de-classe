...
- **Référence de la jauge : `LS_REF = 5 Mo`**, le plafond historique de `localStorage` par origine. Les Chromium récents l'adossent au quota général (2,7–10 Go mesurés selon la machine), mais d'autres navigateurs l'appliquent encore. Rapporter l'occupation au quota global afficherait « 0,04 % » et endormirait la vigilance ; la référence prudente est aussi la seule à produire une barre lisible. Le quota réel reste affiché en contexte, **en disant explicitement que la mémoire locale n'y est pas comptée** — sans cette phrase les deux chiffres semblent se contredire.
- Paliers de la barre : vert < 60 %, `.warn` orange 60–85 %, `.crit` rouge ≥ 85 % de `LS_REF`, avec un conseil d'action adapté (exporter puis alléger). Répartition par section via `_storageBreakdown()` (les 6 plus lourdes, ≥ 1 Ko).
