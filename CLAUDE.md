...
- **Évaluations** (`_seedDemoEvaluations`) : ~11 évaluations réparties sur 5 classes — 6A en a 5 (Type A "Mesures" S1, Type B "Démarche" 3 passations S1+S2, Type C "DS Vol/Masse" S1 avec compétences inline, Type A "Éval finale" S2, **Type D "DS Énergie" S2** — 2 exos / 3 questions, un poids ×2, un élève absent tout-A, une sanction « −2 rendu en retard » qui démontre l'ajustement de note finale), 6B en a 3 (dont 1 Type C noté /40), 5A en a 3 (dont Type B 3 passations + Type C "DST" coef 3), 5B en a 2, 4A en a 1. 3A reste sans évaluation (état vide démontré). Couvre : valeurs A/NN, commentaires, exclusions, `dateIndiv` (rattrapage), `studentRemarks`, coef variés, noteMax variés (10/20/40), compétences inline Type C, niveaux directs Type D.
...
Le label porte une infobulle *« Volet fonctionnel mais encore en cours de finalisation »* — les types **A/B/C/D** de saisie + les deux vues d'agrégation sont opérationnels (Type D : sommative par compétences, cf. section dédiée du bloc Évaluations).
...
- **Type D** — sommative par compétences. **Même structure exercices → questions que le Type C**, mais chaque question évalue **une ou plusieurs compétences par un niveau 1..nb** — jamais des points. Spécification arrêtée avec l'utilisateur le 2026-07-30 (la description historique « sans questions intermédiaires » était fausse) :
  ...
  - **Commentaire par CELLULE (v2.31.0)** — le D annote ses cellules comme les Types A/C, via `clic droit`. ⚠️ **Une cellule D est un couple (question × compétence)**, pas une question : la clé de `ev.notes[sid].comments` est donc **composite**, `mnId|cid` (helper **`_typeDComKey`**), dans la MÊME map que les clés simples des A/C. Conséquences :
    - `_evalCommentsOpen` **découpe la clé** pour titrer « Q1 · C1 · NOM » (sans ça, deux cellules d'une même question sont indiscernables) ; le reste de la plomberie (`_evalCommentsGetList`, ajout, suppression, ré-emballage des strings par `migrateEvalDefaults`) est générique sur la clé et n'a pas bougé.
    - ⚠️ **Nettoyage** : supprimer une question doit emporter la clé simple ET les clés `mnId|*` — d'où **`_evalDropCommentsForMn(rec, mnId)`**, appelé par les 4 sites de suppression (`_evalEditRemoveMiniNote`, `_evalEditRemoveExo`, `_tableurMnDelete`, `_tableurExoDelete`). Un `delete comments[mnId]` seul laissait des commentaires orphelins.
    - **Repère visuel : un coin, pas une pastille** — la cellule fait 36 px, une pastille 💬 masquerait le niveau. Coin de 7 px en haut à droite, `pointer-events:none`, le texte du commentaire passant dans l'infobulle de la cellule. ⚠️ **Sa couleur se dérive du fond** (`_contrastTextColor(bg)`, repli `--margin-red` sur cellule vide) : posé en `--margin-red` fixe il tombait à **1,3:1 sur une cellule de niveau 1** (rouge sur rouge, mesuré) — invisible là où il sert. `_evalTableurDUpdate` le recolore aussi à la saisie, sinon il garde le contraste de l'ancien niveau après un rouge → bleu.
