...
- **Les deux catalogues sont RÉORDONNABLES** (`_comLibMove(kind, idx, delta)` / `_adjLibMove(idx, delta)`), parce que leur ordre est exactement celui des puces et des boutons proposés ensuite : l'enseignant met en tête ce qu'il pose le plus souvent. Boutons **↑ / ↓** construits par le helper partagé **`_reorderBtnsHTML(onUp, onDown, isFirst, isLast, fg)`** — un pas par clic, extrémités `disabled`, même convention que la modale 🔗 Liens (`moveUserLink`) ; pas de glisser, donc rien de particulier à valider au doigt.
  - ⚠️ **`_adjLibMove` lit `S.evalPrefs.adjustPresets` en direct, jamais `_adjustPresets()`** : ce dernier peut renvoyer la constante `ADJUST_PRESETS_DEFAULT`, et la permuter corromprait les défauts pour toute la session (y compris le « ↺ défauts »). Même précaution que `_adjLibRemove`. Un test le vérifie.
  ...
  - ⚠️ Ces lignes, comme les puces du sélecteur de commentaires, ont un **fond LITTÉRAL clair** (`#dcfce7` / `#fee2e2`) : leur couleur de texte est dérivée par `_contrastTextColor(bg)`, la croix ✕ est un littéral `#b03d2e`. Sans ça le texte héritait de `var(--ink-deep)` — de l'ambre en thème sombre, sur du vert et du rose pâles. Mesuré à 14,17:1 / 12,74:1 après correction.
