...
- **Fond de carte de classe** (`levelBgColor`) : teinte **très pâle** (clair) / très sombre (sombre) du niveau. L'ancien aplat pastel saturé ne laissait passer **aucun** texte secondaire coloré (max atteignable 5,4:1 en clair, 3,8:1 en sombre). L'identité du niveau est portée par la bordure gauche saturée (`levelAccentColor`).
