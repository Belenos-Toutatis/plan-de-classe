...
- 💡 `localStorage` est **par origine, pas par page** : tous les projets servis depuis `belenos-toutatis.github.io/` partagent le même stockage. Une clé volumineuse d'un autre outil de l'utilisateur apparaît donc légitimement dans « d'autres clés du même site » — d'où l'importance de les nommer plutôt que de les additionner. Corollaire : usage local (`file://`, `localhost`) et en ligne sont des origines **distinctes**, avec des chiffres différents.
