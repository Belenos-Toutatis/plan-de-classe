...
- **Type D** — sommative par compétences. **Même structure exercices → questions que le Type C**, mais chaque question évalue **une ou plusieurs compétences par un niveau 1..nb** — jamais des points. Spécification arrêtée avec l'utilisateur le 2026-07-30 (la description historique « sans questions intermédiaires » était fausse) :
  ...
  - **Le POIDS d'une compétence** se règle à **deux endroits**, tous deux en édition directe : dans **🏗 Structure** (`_renderCompetenceWeightsD` — chip `AA [2]×` sous chaque question, là où il y a la place de le nommer) et dans l'**en-tête du tableur** (3ᵉ ligne). Le poids vit sur le couple (question × compétence) : une même compétence peut peser 1 ici et 2 là.
    ...
    - Le `×` est placé **après** le nombre (`2×`) : devant, il se lisait comme un bouton de fermeture — signalé en usage réel.
  ...
  - **Hors périmètre v1, à faire** : feuille D dans l'export XLSX/ODS (`_NotesExport` — une éval D y produit une feuille vide type A/C), import QCMcam (sans objet — volontaire).
...
Cinq modes, tous gérés dans `_evalTableurKey`. `Tab` et les flèches ne changent jamais. Une **colonne** est une question en A/C, un couple (passation × compétence) en B, (question × compétence) en D.

| Mode | Déplacement | Usage |
|---|---|---|
| `question` | cellule à droite, puis élève suivant | saisie au fil de la copie |
| `student` | descend dans **la même colonne**, s'arrête au dernier élève | une colonne, sans enchaîner |
| `column` | descend, puis **remonte en haut de la colonne suivante** | corriger une question sur tout le paquet |
| `byquestion` | remplit la **question** pour un élève, puis descend ; question épuisée pour la classe → question suivante, 1ᵉʳ élève | idem, quand une question tient sur plusieurs colonnes (D) |
| `byexo` | idem avec l'**exercice entier** comme bloc | corriger exercice par exercice |

💡 `student` et `column` ne diffèrent **qu'en fin de colonne** : le premier s'arrête, le second enchaîne sur la colonne suivante. En A/C, où une question = une colonne, `byquestion` ≡ `column` — c'est en D qu'il prend son sens.
