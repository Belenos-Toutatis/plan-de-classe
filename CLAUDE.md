...
- **Panneau « Codes de groupes rencontrés »** (`_impRenderCodes`, inséré après le mappage) : une ligne par code distinct — nombre d'élèves, `<select>` Groupe 1/2/3 · Tag · ignorer, abréviation éditable, « tag existant / nouveau tag ». Les choix vivent dans `_imp.codeMap` (`opts.codeMap` pour l'analyse), **remis à zéro à chaque ouverture de la modale**, pas à chaque changement de texte (les mêmes codes reviennent d'un fichier à l'autre). ⚠️ L'utilisateur ne veut pas forcément `BIL-LCE` comme tag mais `BIL` : c'est le panneau qui rend ça possible, pas une heuristique plus fine.
