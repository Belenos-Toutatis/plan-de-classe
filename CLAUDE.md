...
- **Suppression d'une classe = `_purgeClassRefs(id)`** (source unique de vérité, **les deux** branches de `deleteClass` — réelle ET recomposée — l'appellent) : purge `attendance`, `snapshots`, `movedHighlights`, `bulletinRemarques`/`ClassRemarques`/`WorkedItems`, `conseilClasse`, et retire la classe des évals (suppression de l'éval si plus aucune classe vivante via `_evalPrimaryAliveClassId`). La branche réelle supprime d'abord chaque élève via `_deleteStudentInternal`. **Tout nouveau store keyé par classId va dans `_purgeClassRefs`.** Le **rename** d'une classe recomposée (`mvcSave`) migre symétriquement `ev.classId`/`classIds`/`dates`/`slotIds`/`datesManual` (sinon orphelinage silencieux des évals).
...
- **Lint anti-XSS (`test/xss-lint.test.js`)** : échoue si un champ à haute confiance d'être de la donnée utilisateur (`prenom`/`nom`/`nomCourt`/`nomLong`/`descriptif`/`remarque`) est interpolé en clair (sans `_esc*`, accès direct sans appel de fonction) dans une ligne contenant un fragment HTML. Garde-fou anti-régression — lancé par `npm test`.
...
- **`switchClass` purge l'état d'appel de la classe QUITTÉE** : si on change de classe en mode appel ou en édition d'un appel passé, `_absentToday`/`_lateToday`/`_aeshAbsentToday`/`_editingAttendanceId`/`_appelMode`/`_appelSavedSlot` sont vidés AVANT de changer `S.cur` (sinon les absents « fuyaient » sur la nouvelle classe — sids étrangers marqués absents, et écrasement d'un record d'une autre classe à l'enregistrement).
