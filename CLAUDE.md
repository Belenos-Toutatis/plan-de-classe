...
### Familles de tokens ajoutées le 2026-09-07

Presque tous les défauts trouvés venaient de la même faute : **une valeur littérale posée pour le mode clair, sans variante sombre**. D'où quatre familles de tokens, chacune déclinée dans `:root`, dans `html[data-theme="dark"]` et dans la neutralisation `@media print`.

| Token | Sert à | Remplaçait |
|---|---|---|
| `--alert-bg/-bd/-fg` | indisponible, absent, élève parti | `#fee2e2` + `#991b1b` (1,14:1 en nuit) |
| `--caution-bg/-bd/-fg` | liste des non placés | `#ffeeba` (1,21) |
| `--notice-bg/-bd/-fg` | bandeau RGPD, rappels, cartes d'information | `#fef9e7` + `#7d3c00` (1,57) |
| `--aesh-bg/-bd/-fg` | AESH, plan et Vue Élève | `#fde2f3` + `#880e4f` (1,38) |
| `--info-bg/-bd` | surfaces bleutées d'information | `#eaf4ff` (1,25) |
| `--link-fg`, `--ok-fg`, `--danger-fg` | liens et titres de carte | `#3498db` (3,15), `#1b5e20`, `#7d2017` |
| `--danger-bg` / `--danger-on` | pastilles rouges pleines (💬, incompatibilité) | `var(--margin-red)` + `#fff` figé (3,87) |
| `--cell-meta` | n° de place, pastille tablette, croix, sur une cellule TEINTÉE | `--pencil` (3,88 sur un pastel de tag) |
| `--g1-fg/--g2-fg/--g3-fg` | couleurs de groupe utilisées en ENCRE | `--g1/2/3` (2,35 en nuit) |
| `--st-*-fg` (8) | dispositifs en encre (cases de Modifier élève) | accents des pastilles (2,33–3,28) |

⚠️ **Un accent utilisé en FOND et en TEXTE a besoin de deux valeurs** — déjà noté pour `.spec-ppre`, vérifié à nouveau à ses dépens : éclaircir le `--margin-red` du thème sombre a rendu lisibles l'astérisque « champ obligatoire » et les indices, **et fait tomber à 3,24:1** les pastilles qui s'en servent de fond. D'où le retour à la valeur d'origine plus un `--danger-fg` distinct pour l'encre.

⚠️ **`--pencil` est calibré sur `--paper`, pas sur les autres surfaces** : 5,2:1 sur le papier, mais 4,26 sur `--paper-warm`, 4,12 sur `--paper-deep` et 3,88 sur un pastel de tag. `--pencil-soft` est passé de `#606f86` à `#5b6a80` pour tenir sur `--paper-warm` ; sur une cellule teintée ou un pied de tableau, utiliser `--cell-meta`.

⚠️ **Un gris de luminance moyenne ne porte NI l'encre sombre NI le blanc** : `#7f8c8d` plafonnait à 4,47 des deux côtés dans `TAG_COLORS`. `_contrastTextColor` ne peut rien y faire — c'est la teinte qu'il faut changer (éclaircie en `#95a5a6`). Même raisonnement pour le **dégradé de rang** du Bilan des notes, qui traverse une bande d'oranges et de bleus où la clarté fixe échouait des deux côtés : `_rankGradientColor` corrige désormais la **clarté** (jamais la teinte, qui porte le sens) dans le sens le moins coûteux, et laisse `_contrastTextColor` choisir l'encre.
...
- **Cartes de classe (onglet Classes)** — `levelBgColor` garde un **aplat saturé** : une teinte par niveau + une variation de luminosité par classe au sein du niveau. ⚠️ **Ne pas diluer ce fond** : repérer le niveau d'un coup d'œil est sa fonction première (une tentative de fond très pâle a été rejetée par l'utilisateur — « toutes les classes sont très similaires quel que soit leur niveau »). Comme aucune couleur de texte **fixe** ne tient 4,5:1 sur toute la gamme (max 5,4:1 en clair, 3,8:1 en sombre ; 1,4–2,6:1 pour des accents comme ♂/♀), la lisibilité vient de la **dérivation du texte depuis le fond de chaque carte** : `renderClasses` pose `--cc-fg:${_contrastTextColor(bg)}` en inline, et `.cc` / `.cc-name` / `.cc-meta` lisent `var(--cc-fg, …)` (7,9–11,3:1 mesurés). Les compteurs ♂/♀ et 📄 gardent leur code couleur (bleu / rose / sarcelle, tokens `--civ-m` / `--civ-f` / `--agr-fg`) grâce à la classe **`.cc-chip`** : une pastille à **fond opaque `var(--paper)`**, qui isole la couleur du fond de niveau et la rend indépendante de la carte (5,1–6,3:1 en clair, 8,3–9,3:1 en sombre). Un bloc `@media print` repose les valeurs du thème clair sur ces pastilles — sinon un thème sombre actif à l'écran imprimerait un bleu ciel délavé sur papier blanc. `levelAccentColor` ajoute une bordure gauche saturée **identique pour toutes les classes d'un même niveau** — le niveau se lit sur la bordure, la classe sur la nuance du fond.
  - **Récap photocopies** (bas de l'onglet Classes, et « Bilan des classes » imprimé — `printClassesBilan`, tableau par classe ET récap par niveau) : deux familles de colonnes à ne pas confondre. Les **élèves** (« Non aménagés » / « Aménagés » = PPRE, PAP, PPS, ULIS+, UPE2A+, PAI parmi les présents) et les **documents à tirer** (« 📄 Docs non aménagés » = présents sans agrandissement, `docStd` · « 📄 Docs aménagés » = agrandissements, ex-colonne « Agrandissements », renommée le 2026-09-21 à la demande de l'utilisateur). `_classCounts` expose `agrPresent` (agrandissements parmi les présents) et `docStd = present − agrPresent`. ⚠️ Les docs aménagés restent comptés sur **tous** les inscrits, hors-inclusion compris (un ULIS/UPE2A hors classe qui a un agrandissement en a besoin quand il vient) : les deux colonnes peuvent donc dépasser les présents d'une unité, c'est voulu et dit dans l'infobulle. Les puces `📄 N agrandissements` des cartes décrivent l'attribut élève et gardent leur nom.
...
transport entre les machines via `github.com/Belenos-Toutatis/plan-de-classe`.
...
corruption** : les « objets fantômes » qu'il liste sont du résidu normal de `git stash` et de
`commit --amend`, datés de mai et juin, sans rapport avec le 19 juin. ⚠️ Ne pas confondre les
deux — un fantôme n'est pas un dégât.
...
à la RACINE du dossier synchronisé.** Un `.sync-exclude.lst` déposé dans un sous-dossier
comme celui-ci est **inerte** : il n'est pas lu, il est synchronisé comme un fichier
ordinaire. (Erreur commise puis corrigée le 2026-09-10 — le nom apparaît dans le binaire du
client, ce qui dit qu'il est connu, pas où il est cherché.)

La règle est donc, à la racine, un chemin relatif :

```
gestion élèves/Plan de classe
```

💡 Ce fichier étant lui-même synchronisé, il atteint les autres machines tout seul. Le client
l'a relu **sans redémarrage**.

⚠️ **Le dossier de DONNÉES n'est pas visé** : `gestion élèves/Plan de classe json/`
(88 fichiers) est un dossier VOISIN, pas un sous-dossier. Le motif ne l'atteint pas et sa
synchronisation continue — vérifié par un témoin déposé dedans, qui est bien parti.

### Comment le VÉRIFIER — deux pièges de mesure

⚠️ **Compter les entrées du journal de sync ne prouve rien** : la table `metadata` enregistre
ce qui a été synchronisé par le passé, elle ne se vide pas quand on exclut.

⚠️ **Lire ce journal avec `immutable=1` renvoie un instantané périmé** — SQLite ignore alors
le WAL. Copier les trois fichiers (`.db`, `-wal`, `-shm`) et interroger la copie.

**Le seul test valable est une SONDE avec son TÉMOIN** : un fichier déposé dans le dossier
censé être exclu, un autre dans un dossier certainement synchronisé, et on regarde lequel
arrive. Sans témoin, « rien n'est arrivé » peut vouloir dire que le client ne tournait pas.

⚠️ **Corollaire** : ce dossier n'est plus sauvegardé par Nextcloud. Le filet, c'est GitHub —
donc **le travail non commité n'est protégé par rien**. (`plan de classe.html.bak`, la copie
manuelle avant modification lourde, reste locale et non sauvegardée : c'est son rôle.)
...
**Codes de groupes à la Pronote** — une cellule « Groupes » exportée de Pronote est une **liste** : `3A-GP1,3A-LATIN,3ABC-CATHO`, `3B-BIL-LCE,3B-GP1,3B-NBIL-DNL`. Les colonnes `groupe` **et** `tags` passent par ce chemin : `_impSplitCodes` découpe sur `, ; | /`, `_impStripClassPrefix` retire le préfixe de classe (un id connu d'abord, sinon le motif `chiffres + lettres + tiret`, une seule fois — `3B-BIL-LCE` → `BIL-LCE`), puis `_impDefaultCodeInterp` : `GP1`/`G1`/`GROUPE1` → groupe, tout le reste → **tag abrégé au 1er segment** (`BIL-LCE` → `BIL`, `NBIL-DNL` → `NBIL`), tronqué à 6 (limite des tags). Un nombre nu (`2`) reste un groupe direct.
- **Panneau « Codes de groupes rencontrés »** (`_impRenderCodes`, inséré après le mappage) : une ligne par code distinct — nombre d'élèves, `<select>` Groupe 1/2/3 · Tag · ignorer, et **la liste des tags** à poser (`interp.abbrs`, saisie libre « BIL LCE » / « BIL, LCE » / « BIL+LCE » parsée par `_impParseAbbrs` ; ✚ marque ceux qui seront créés). **Un même code peut poser plusieurs tags** — demandé en usage réel ; l'ancienne forme `{ abbr }` reste lue par `_impInterpAbbrs`.
- ⚠️ **`_impRefresh` préserve le défilement et le focus** à travers le re-rendu : les panneaux sont reconstruits par `innerHTML`, donc sans précaution la modale remontait en haut à chaque changement d'interprétation — pénible quand on règle un code en bas de la liste (signalé en usage réel). On mémorise `scrollTop` de `#mi .mb` et de `#imp-codes-scroll`, plus le contrôle actif (`data-code` / `data-field`), et on les remet après rendu (`focus({ preventScroll: true })`). **Tout nouveau contrôle du panneau doit porter ces deux attributs** pour bénéficier du rappel de focus. Les choix vivent dans `_imp.codeMap` (`opts.codeMap` pour l'analyse), **remis à zéro à chaque ouverture de la modale**, pas à chaque changement de texte (les mêmes codes reviennent d'un fichier à l'autre). ⚠️ L'utilisateur ne veut pas forcément `BIL-LCE` comme tag mais `BIL` : c'est le panneau qui rend ça possible, pas une heuristique plus fine.
- **Colonnes devinées** : `_impLooksLikeGroupCodes` reconnaît une colonne sans en-tête dont la moitié des cellules contient un code `GPn`.

**Classes inconnues créées à l'import** (case « Créer les classes inconnues », défaut coché ; `opts.createClasses`) : la valeur de la colonne Classe qui ne matche ni id ni nom devient une classe via `_createClassBare(id, nom)` — le cas par défaut de `createClass` extrait (salle 4×10 neuve, `setupClassAccessors`), sans `pushUndo` ni `save` (l'import fait les deux). **L'id est compacté par `_impClassIdFromLabel`** (« 3ème C » → `3C`, « 2nde 3 » → `23`, « 1ère S2 » → `1S2` : l'ordinal collé au niveau est retiré), **le libellé complet reste le nom**. Résumé « ✚ Classes à créer : … » sous l'aperçu et ✚ dans la cellule Classe — mais **pas d'avertissement par ligne** (la cellule et le résumé suffisent, le ⚠ est réservé à ce qui demande un regard). **Salle des nouvelles classes** (`_impRoomChoiceHTML`, `_imp.room = { mode, salleId, name }`) : plusieurs classes partagent en général la **même salle physique**, or créer une salle par classe importée obligeait ensuite à tout rattacher à la main (signalé en usage réel). Trois choix, sous la ligne des classes à créer : une **salle existante** (défaut : celle de la classe courante, sinon la 1re du catalogue), une **nouvelle salle commune** à nommer (défaut quand le catalogue est vide — premier lancement), ou **une salle 4×10 par classe** (l'ancien comportement). `_createClassBare(id, nom, yr, salleId)` prend la salle à rattacher ; sans elle il en crée une. Case décochée → repli sur la classe courante avec avertissement, comme avant. ⚠️ **Aucune classe courante n'est requise** pour ouvrir la modale ni pour importer — au premier lancement, ou après un « Tout effacer », il n'y a aucune classe et c'est le fichier qui doit les créer. L'ancien garde « Sélectionnez une classe » a été retiré (il rendait le bouton inopérant précisément dans ce cas, signalé en usage réel). Sans classe courante, l'aperçu l'annonce, les lignes sans classe sont rejetées (« aucune classe cible »), et après l'import la **première classe servie devient la classe courante** (`switchClass`), sinon l'onglet restait vide malgré un import réussi.

**Sélection des classes du fichier** (v2.58.0, `_impRenderClasses`, panneau `#imp-classes` inséré après les codes de groupes) : un export Pronote couvre souvent tout un niveau, voire l'établissement. Une case par classe rencontrée, avec son effectif et son statut — *existante*, *✚ sera créée*, *inconnue → classe courante* (création décochée), *Sans classe* — plus ☑ Toutes / ☐ Aucune / « Seulement <classe courante> ». Les lignes sont regroupées par **`rec.classKey`** : id de la classe reconnue ou à créer, sinon `'?' + libellé brut`, sinon `''`. Les cases décochées vivent dans `_imp.exclude` (Set de clés), remis à zéro à l'ouverture, au changement de fichier (signature) et après l'import ; `opts.excludeClasses` les passe à l'analyse, qui marque ces lignes `excluded` (non importées, **masquées de l'aperçu**, comptées à part). ⚠️ **`newClasses` est calculé en fin d'analyse depuis les lignes NON exclues** — une classe décochée n'est jamais créée. Basculer « Créer les classes inconnues » change la clé d'une classe inconnue (`?3ème C` ↔ `3C`) : sa case décochée ne suit pas, assumé.

**Analyse pure** `_impAnalyze(text, { mode, mapping, header, codeMap, createClasses, excludeClasses, skipDup, defaultClassId })` → `{ mode, sep, header, rows, cols, mapping, records, codes, newClasses, classGroups }`. Ne touche pas au DOM (testable) ; `records[]` = `{ line, nom, prenom, groupe, classeId, classeRaw, targetClassId, civilite, tagAbbrs, statuses, arrivalDate, notes, ok, dup, warnings[] }`. Une classe inconnue → classe courante **avec avertissement** (pas de rejet). Doublons : dans le fichier (nom+prénom+classe, sans accents ni casse) → 2ᵉ ligne rejetée ; déjà présent dans la classe → rejeté si la case « Ignorer les élèves déjà présents » est cochée (défaut), sinon importé et signalé.
...
- **Dimensions cibles MESURÉES, pas figées** (`_pickMeasureCard`, `PICK_CARD_MIN_W/H = 300/240` = minimums). L'ancien `cardW = 300, cardH = 240` en dur + `overflow:hidden` **tronquait les boutons** dès que la police rendait plus large que sur la machine de développement (réglage de taille de police du navigateur, écran tactile où le bloc `@media (pointer: coarse)` porte les `.btn-sm` à `.95em` / `9px 13px` → 375 px nécessaires), et le bandeau « ↻ Cycle terminé » poussait déjà les boutons sous le bord bas même en rendu standard (250 px nécessaires). La mesure se fait carte invisible, sans transition, `transform:none` (le `scale(.7)` de repos fausserait `getBoundingClientRect`) : **largeur** = minimum élargi de ce dont la rangée de boutons déborde quand on lui interdit le retour à la ligne — sur les DEUX conteneurs flex (`.pf-actions` ET `.pf-secondary`, sinon le second replie ses propres boutons et rien ne déborde) — et non le max-content de toute la carte (la ligne de compteur l'élargirait alors qu'elle peut passer sur deux lignes) ; **hauteur** = celle du contenu à cette largeur. `+1 px` sur chaque mesure : `scrollWidth` est arrondi à l'entier et 0,4 px de débordement suffisaient à renvoyer un bouton à la ligne. Filet CSS : `.pf-actions` / `.pf-secondary` en `flex-wrap:wrap`, boutons en `white-space:nowrap`. ⚠️ Le bornage au viewport (`innerWidth − 24`) ramène tout au minimum si le viewport vaut 0 — c'est le cas d'un panneau d'aperçu **masqué** (piège de test rencontré, pas un cas réel : émuler une taille de fenêtre avant de mesurer).
...
- ⚠️ **Le tiers-temps n'est JAMAIS écrit plus petit que le temps de la classe.** Première version fautive, corrigée sur retour de l'utilisateur : *« les élèves qui ont besoin d'un tiers-temps sont aussi ceux qui ont besoin d'avoir des choses écrites plus grandes »*. Or ce décompte-là est précisément celui qu'ils lisent. Les deux sont donc à la **même taille** tant que la classe compose ; quand le tiers-temps est affiché, les deux rétrécissent d'autant, **jamais lui seul**. Une fois le temps commun écoulé (`mainDone`), c'est le principal qui s'efface — il est figé à 00:00 — et le tiers-temps qui prend toute la place.
- **La taille est bornée en `vw` autant qu'en `vh`** : sur un vidéoprojecteur large, c'est la largeur qui contraint, et des chiffres dimensionnés en `vh` seul débordent de l'écran. Mesuré à `min(30vh, 17vw)` : **324 px par chiffre en 1920×1080 avec les deux décomptes**, soit exactement la taille d'un décompte seul — la place verticale libre absorbe la seconde ligne, afficher le tiers-temps ne coûte donc rien en lisibilité sur un 16:9. Vérifié sans débordement sur 9 géométries, du 1920×1080 au 380×280.
- Les deux valeurs sont dans une **grille étiquette | valeur** (`#tpw-times`), étiquettes « Classe » et « Tiers-temps » écrites en toutes lettres — plus lisible qu'un `+⅓` pour des élèves à qui la fenêtre est projetée. L'étiquette de la classe est masquée quand le tiers-temps n'est pas affiché (`body:not(.tt-on) .tpw-lbl`), il n'y a alors rien à distinguer.
...
**Ctrl+Z / Ctrl+Y depuis n'importe quel écran ou modale.** Le gestionnaire clavier global ne laisse la main au navigateur que si la cible est une **saisie de texte** (`_isTextEntryTarget` : input texte/nombre/date…, textarea, contenteditable) — là, Ctrl+Z est l'annulation native de la frappe. ⚠️ Cases à cocher, radios, curseurs, boutons et `<select>` gardent le focus après un clic sans avoir d'undo natif : l'ancien garde « `tag === 'INPUT'` » y rendait Ctrl+Z **muet** (cocher un aménagement dans Modifier puis Ctrl+Z ne faisait rien — trouvé à l'audit). Après undo/redo, l'onglet courant est re-rendu ET les **modales de consultation ouvertes** le sont aussi via `_MODAL_RERENDER` / `_refreshOpenConsultModals` (fiche, historique, rappels, tags, liens, vue d'ensemble, stats d'appel, appels passés, snapshots, mentions de conseil, disciplines, compétences) — sans ça le modèle revient mais la fenêtre reste figée sur l'ancien état. Les modales **à formulaire** (`me`, `ms`, `meval-new`…) n'y sont volontairement pas : les re-rendre effacerait la saisie en cours. **Tout nouveau panneau de consultation s'ajoute à `_MODAL_RERENDER`.**
...
- **Clic sur le nom** → 🪪 fiche complète de l'élève (`openFiche`). Le reste de la ligne garde la sélection par clic & glisser.
...
- **Sélection multiple par clic & glisser** sur les lignes (sans case à cocher). ⚠️ Le glisser est suivi par un **`mousemove` global** (`elementFromPoint` + comblement de la plage depuis `_dragSelectLastSid`), **plus par `onmouseenter`** : ce dernier se déclenche aussi quand le tableau bouge sous un curseur immobile. Défaut signalé en usage réel : la barre d'actions en lot apparaissait au-dessus du tableau pendant le clic, le faisait coulisser d'une ligne ou deux, et plusieurs élèves se retrouvaient sélectionnés.
- **Barre d'actions en lot `#el-bulk-bar` FLOTTANTE** (`position:fixed` en bas, `body.el-bulk-on` posé par `updateEleveBulkBar`) : elle ne pousse plus rien, reste accessible en bas d'une longue liste ; le tableau reçoit un `padding-bottom` pour que la dernière ligne ne passe pas dessous, les toasts sont remontés (seulement quand l'onglet Élèves est affiché, via `:has(#tab-eleves.on)`), et la barre se décale à gauche quand le widget sonomètre/minuteur est ouvert.
- **Clic droit (ou appui long tactile 550 ms) sur le n° de position** → modale `mseatpick` (`openSeatPicker(e, sid)` / `renderSeatPicker`) : plan de la salle en orientation Plan Prof, la place actuelle de l'élève en bleu (`.spk-me`), clic sur une case vide = déplacer, sur un autre élève = échanger (`↔` ; `⚠` si l'élève n'a pas de place — l'occupant devient non placé, comme la saisie du numéro), bouton « ↩ Retirer de sa place ». Cases AESH et sans table inertes. **Sélecteur de salle** (`#mseatpick-salle-row`) seulement si la classe a ≥ 2 salles ; y choisir une salle appelle `setActiveRoom` (= la salle ACTIVE de la classe change, la colonne Position suit) ; un 📍 marque les salles où l'élève a une place. Les mutations passent par `applyEleveSeat` (tablettes qui suivent, swap) + `_recordMovesFromDiff` + `_maybeShowViolationsModal`, comme `onPosBlur`. Dans `_MODAL_RERENDER` (Ctrl+Z re-rend le plan). Bloqué en consultation de snapshot.
...
- `_attachSalleToClass(cls, salleId, makeActive)` / `_detachSalleFromClass(cls, salleId)` — helpers **génériques** (n'importe quelle classe), sans `pushUndo` ni `save`, désormais aussi utilisés par `addSalleToClass` et `confirmRemoveSalleFromClass`. Le détachement refuse la **dernière salle** d'une classe (`{ ok:false, reason:'derniere' }` ; la case est désactivée avec l'explication en infobulle) et renvoie le nombre d'élèves placés pour que l'appelant confirme **avant** d'appeler.
- ⚠️ Le conteneur `#cfg-salle-classes` porte son `display:flex` **en ligne** : le re-rendu doit poser `box.style.display = 'flex'`, pas `''` — vider la valeur retire le flex et chaque puce retombe sur sa propre ligne (17 lignes pleine largeur, vu en usage réel).
...
3. **Tags** — sélection multi-cases puis clic sur un bouton de tag défini (modifie `room.posTagId`). Bouton `⚙ Gérer les tags` ouvre la modale `mtags`.
   ...
   - ⚠️ **La couleur d'un tag est choisie par l'UTILISATEUR** : le blanc n'y tient pas toujours. Les boutons de tag de la barre d'action de Config Salle et les puces de filtre de la modale Classe recomposée forçaient `color:#fff` — 2,56:1 sur le gris de la palette, 2,85 sur l'orange, 2,87 sur le vert, 3,28 sur le sarcelle. Ils dérivent désormais leur encre par `_contrastTextColor`, comme les pastilles. Toute la palette par défaut passe alors 4,67:1 au minimum.
...
- **Type D** — sommative par compétences. **Même structure exercices → questions que le Type C**, mais chaque question évalue **une ou plusieurs compétences par un niveau 1..nb** — jamais des points. Spécification arrêtée avec l'utilisateur le 2026-07-30 (la description historique « sans questions intermédiaires » était fausse) :
  ...
  - 💡 **État vérifié le 2026-09-20 au clavier réel, pour les quatre types** : A et C passent par `_evalTableurRefreshFooterStats` (colonnes, Σ exercices, total brut, note, compétences — complet depuis l'origine) ; D par `_evalTableurDRefreshFooter` ; B par `_evalTableurRefreshFooterStatsB`. ⚠️ Piège de mesure : une cellule passée de `NN` à 1 déplace la moyenne de 2,93 à 2,86, soit « 2,9 » dans les deux cas — tester avec un écart net (4 → 1) avant de conclure à un pied figé.
  - ⚠️ **Le pied du tableur B non plus** (v2.56.5) : `_evalTableurRefreshFooterStatsB` ne rafraîchissait que la colonne (passation × compétence) saisie ; la μ de la **Note /20** (`[data-stat-b-note]`) et les niveaux moyens par compétence (`tfoot [data-stat-comp]`) restaient figés jusqu'à la réouverture. Même remède qu'en D.
  ...
  - **Colonnes de bilan de compétences en PASTILLES** (`_compPillHTML`, comme le Type C), pas en aplat de fond sur le `<td>`. Vaut pour les colonnes globales (`.d-comp-cell`) et par exercice (`.d-exocomp`), au rendu comme au rafraîchissement en place. ⚠️ Ce n'est pas qu'esthétique : la couleur passe dans `.comp-pill`, **donc dans le champ du mode Sobriété** — un aplat sur le `<td>` y échappait.
    - ⚠️ **Les TROIS tableurs à colonnes de bilan (B, C, D) posent la pastille**, et le **rendu** doit poser la même chose que le **rafraîchissement en place**. Le Type B faisait exception : `_evalTableurRenderB` peignait le fond du `<td>` alors que `_evalTableurRefreshRowDerived` (partagé avec le C) y posait une pastille — donc pastilles pendant la saisie, aplats à la réouverture du tableur, **signalé en usage réel**. Son aplat portait en prime un `Math.min(3, …)` figé à quatre niveaux, que `_compPillHTML` fait disparaître. **Quand un rendu et son rafraîchissement ciblent la même cellule, ils doivent produire le MÊME balisage** — sinon la divergence n'apparaît qu'après un cycle fermeture/réouverture, hors de portée d'un test qui ne regarde qu'un seul des deux chemins.
...
### « ⊘ Non faite par cette classe » — question (A/C) ou passation (B) sautée pour une classe (v2.55.0)

Sur une éval multi-classes, une classe peut ne pas avoir passé une mini-note, une question ou une passation. `mn.skippedClasses = { [classId]: true }` (idem `pass.skippedClasses`) le consigne. Demandé en usage réel : une colonne vide **comptait déjà pour rien** (`_computeStudentEvalNoteRaw` ignore les cellules vides), mais rien ne distinguait « pas encore saisie » de « pas faite », et plusieurs mécanismes se comportaient comme si la colonne était attendue.

- **Helpers** (près de `_evalCollectAbsentsForDateSlot`) : `_subSkippedFor(sub, classId)` · `_subSetSkipped(sub, classId, on)` (supprime la map quand elle se vide) · **`_evalStuClassId(ev, sid)`** — la classe de l'élève *du point de vue de l'éval* (sa classe d'inscription si l'éval la couvre, sinon la classe de l'éval qui le compte dans son roster : classes recomposées) · `_evalSkippedSummary(ev)` · `_evalSkipPillHTML` · `_evalToggleSubSkipped` (une entrée d'undo, no-op si inchangé, recalcule la date auto).
- ⚠️ **La map est keyée par classId → elle est dans `_forEachEvalPerClassMap`** (sous-niveau mn/passation) : purge et renommage de classe en héritent, le test de couverture des purges la porte.
- **Calcul** — tous les sites qui bouclent sur `miniNotes` / `passations` pour UN élève filtrent par `_subSkippedFor(sub, _evalStuClassId(ev, sid))` : `_computeStudentEvalNoteRaw`, `_computeStudentExoSum`, `_computeStudentNoteBrut`, `_computeStudentEvalNoteB` (les deux pondérations), `_computeStudentCompetenceLevel`, `_evalCompetenceLevel`, `_evalStudentLevelForComp`, `_evalStudentCompetenceLevel`, `_aggregateStudentCompetence`, `_studentHasAnyDataInEval`, `_studentNoteAbsenceReason`, `_evalNoteTipB` (qui annonce « ⊘ N passations non faites — hors calcul »). Barème : la classe est notée sur ce qu'elle a fait puis ramenée `/noteMax` — même comportement qu'une colonne vide, confirmé par l'utilisateur. **Une valeur restée dans une colonne sautée est ignorée, pas effacée** (réactiver la retrouve).
- **Par classe** — `_evalListEvaluatedCompetences(ev, sortMode, classId)` et `_evalCompetencesEvaluated(ev, classId)` prennent une classe optionnelle (une compétence portée seulement par des passations sautées n'est pas évaluée pour cette classe) ; `_evalAutoUpdateDate` et `_evalAutoFillAbsentsForMn/Pass` sautent les colonnes sautées ; `_renduListCols(ev, classId)` (défaut `_renduState.classId`), l'export ENT et les feuilles d'éval XLSX/ODS omettent la colonne ; le plan de classe du Type B (`_evalPlanOverlayOpen/Render`) l'ignore.
- **Tableur : colonne REPLIÉE, pas masquée** (arbitrage utilisateur) — `th.meval-head-skip` / `td.meval-cell-skip`, 22 px, hachures `--paper-warm`/`--rule-line`, code écrit à la verticale, aucun input. Elle reste visible pour dire qu'elle existe pour les autres classes ; le clic droit sur son en-tête la réactive. En Type B, les N colonnes (passation × compétence) se replient en UNE ; la cellule porte `data-ci` = premier indice et `ci` avance de N pour que `numCols` reste vrai.
  - ⚠️ **`_evalTableurKey` enjambe les colonnes sans input** : le calcul du pas est extrait dans `step(ri, ci)` et rejoué jusqu'à trouver un champ (garde `numCols × (numRows+1)`). `_tableurBlockAt` compte aussi `td.meval-cell-skip[data-ci]`, sinon un exercice dont une question est sautée se scinderait en deux blocs pour « Exercice par exercice ». Sur une colonne repliée le bloc est introuvable → repli « cellule suivante », le pas d'après retrouve un bloc. Vérifié : Tab, Maj+Tab, Entrée en `byquestion` et `byexo`.
  - Les Σ d'exercice et `maxBrut` excluent les questions sautées de la classe (rendu ET `_evalTableurRefreshFooterStats`, via `_evalTableurActiveCls`).
...
- ⚠️ **La date présélectionnée doit être lue par `_passDateFor`, pas `activePass.date`** (v2.54.1) : la date globale ne matchait aucune clé dès qu'une passation portait une date par classe (édition dans l'en-tête du tableur, éval multi-classes) — le plan s'ouvrait **vide sans un mot**, signalé en usage réel. Repli sur la date la plus récente si la clé n'existe pas.
- **Le rendu ne sort plus en silence** : `_emptyStateHTML` pour « aucune passation à cette date », « aucune salle active », « aucun élève placé dans <salle> », « aucune compétence sur cette passation ». Les passations « ⊘ non faites par cette classe » et le filtre de groupe du tableur y sont appliqués (hors groupe : estompés, `pointer-events:none`).

### En-tête des tableurs A/B — sélecteur de créneau `select.tableur-slot-sel` (v2.55.1)
...
- **`--tt-fg` est un token neuf** (violet du tiers-temps), déclaré dans les trois blocs — `:root`, thème sombre, neutralisation `@media print` — comme l'exige la règle du design system ; un test le vérifie désormais. `--agr-fg` existait déjà.
- ⚠️ **L'encre des puces actives s'inverse avec le thème.** `--agr-fg` et `--tt-fg` sont **foncés en clair et clairs en nuit** : le blanc posé sur la puce active tombait à **1,94 et 2,33:1** sous le mode sombre (mesuré). D'où une surcharge `html[data-theme="dark"] .amchip.…on { color:#1a252f }`. Même famille que `.spec-ppre` et que le bouton **+⅓** du minuteur — tout accent qui sert de FOND à du texte a besoin d'une encre par thème.
...
## 🔒 Mode confidentiel (montrer l'écran à un parent)

Bouton **🔒** dans le header (`#btn-conf`, `toggleConfMode()`), à gauche du minuteur. Masque sur **Élèves, Plan (Prof), Devoirs (tableurs de saisie), Bilan des notes, Bilan des compétences** tout ce qui relève de la vie scolaire — oublis, travail non fait, absences/retards, aménagements, groupe, tags, notes, niveaux de compétence, remarques, mentions de conseil, rappels — **sauf pour l'élève révélé**. Arbitrages de l'utilisateur (2026-09-25) : on révèle un élève en **cliquant sur son nom** (ou via le sélecteur du bandeau) ; les **noms** des autres restent visibles ; les **statistiques de classe** (pieds de tableaux, remarque de classe) restent toutes visibles ; le mode est **persisté** (`localStorage.planClasse_confMode = {on, sid}`) pour qu'un F5 devant le parent ne réaffiche rien.

- **État** `_conf = { on, sid }` (var, lu pendant `init()`). `_confHides(sid)` · `_confNameClick(sid)` (en mode : révèle l'élève, ou le **désélectionne si on clique à nouveau sur l'élève déjà affiché** — tout redevient masqué —, et renvoie `true` → l'appelant n'exécute pas son action habituelle ; hors mode → `false` → fiche / historique comme d'habitude). Motif utilisateur : recevoir un parent sans savoir encore de qui il est le parent. Sur le plan, la case de l'élève affiché porte le même clic (hors boutons internes et hors mode appel) · `_confReveal(sid)` · `_confRefresh()`.
- **Tableaux : masquage 100 % CSS**, généré par `_confUpdateStyle()` dans `<style id="conf-dyn-style">` : `tr[data-sid]:not([data-sid="<révélé>"]) > td:not(:first-child):not(.conf-keep)` → fond hachuré neutre, texte transparent, enfants `visibility:hidden`, `pointer-events:none` (coupe aussi les infobulles, qui portent de l'information). Aucune reconstruction au changement d'élève. ⚠️ **Contrat pour tout nouveau tableau par élève** : lignes `tr[data-sid]`, **1re cellule = le nom** (restée visible, `onclick="_confNameClick(sid)||…"`), cellules non confidentielles marquées `.conf-keep` (ex. la Position dans Élèves). Les pieds de tableau n'ont pas de `data-sid` : ils restent visibles, c'est voulu. Le Bilan des compétences n'avait pas de `data-sid` sur ses lignes : ajouté.
- **Plan : rendu dépouillé**, pas masqué — couleurs de groupe/tag/genre, badges, compteurs, état d'appel, alertes de contrainte, surlignage d'aménagement sont posés en classes et en contenu, un masque CSS en laisserait fuiter. `buildCell` renvoie `_confBuildHiddenCell` (place, nom, tablette, 🔒) ; clic ou clic droit = révéler ; pas de glisser. Liste des non placés : badges masqués par CSS, clic = révéler. Bandeau 📣 messages à la classe caché (un message peut nommer un élève).
- **Bandeau `#conf-banner`** (dans `#topbar`, tokens `--notice-*`), affiché seulement sur les 5 onglets concernés via `:has(#tab-….on)` : sélecteur de l'élève révélé (`_confRenderBanner`, rafraîchi par `showTab` et `switchClass`) + « 🔓 Quitter ».
- `init()` pose le style et `body.conf-on` **avant le premier rendu**.
- **Onglet Élèves** : le **texte du nom ouvre TOUJOURS la fiche complète**, même en mode confidentiel (`_confOpenFiche` : l'élève devient l'élève affiché, puis `openFiche`) ; le **reste de la cellule du nom** porte le clic afficher / masquer (`<td onclick="_confNameClick(…)">`). En mode, `onEleveRowMouseDown` ne démarre pas de sélection dans cette cellule.
  - ⚠️ **Régression corrigée (v2.61.1)** : `onEleveRowMouseDown` ignore les clics sur `[onclick]` ; le `onclick` posé sur la cellule du nom y faisait avorter TOUTE sélection partant de la 1re colonne, **même hors mode**. D'où `[onclick]:not(td)`. Tout `onclick` ajouté sur une cellule d'une ligne sélectionnable doit tenir compte de ce test.
- **Bouton 🔒 CLIGNOTANT** quand le mode est actif (demande de l'utilisateur : l'affichage inhabituel doit sauter aux yeux), `@keyframes confBlink`, ~0,9 Hz. ⚠️ Le rouge est une **ombre intérieure** (`box-shadow: inset 0 0 0 40px …`), pas un `background` : la règle du design system `.hdr-btn { background: var(--paper) !important; border: … !important }` gagne sur toute animation — une déclaration `!important` bat les keyframes. Même piège pour toute future pastille animée du header.
- **Vue d'ensemble** (`moverview`, bouton 📊 de l'onglet Élèves, tous les élèves de toutes les classes) : masquée comme les tableaux (lignes `tr[data-sid]` ; la 1re cellule est la CLASSE, le nom est marqué `.conf-keep` et porte le clic afficher / masquer). ⚠️ En mode, les **filtres « Min. 🚫/⏰ » et les tris par compteur sont ignorés** (tri classe puis nom) : la présence d'un élève dans une liste filtrée, ou son rang dans un tri par absences, trahirait ce qu'on masque. `_confRefresh` la re-rend si elle est ouverte.
- **Appels passés** : la liste (`matt`) ne montre que des comptes par séance, elle reste telle quelle. Le **détail** (`mattDet`) ne nomme que l'élève affiché — les autres absents/retards deviennent une ligne « 🔒 N autres élèves — noms masqués » (`_confMaskedLi`), les nombres de la séance restent ; « 📋 Copier la liste » est bloqué. Le **plan de l'appel** (`mattPlan`) n'affiche l'état absent/retard que pour l'élève affiché, et la liste des absents non placés est filtrée de même.
- **Stats par élève** (`mattStats`) : masquées comme la Vue d'ensemble (`tr[data-sid]`, nom `.conf-keep` cliquable) ; « 0/0 masqués » et tris par compteur ignorés ; export CSV bloqué.
- `_confRefresh` re-rend `moverview`, `mattStats`, `mattPlan` et `mattDet` s'ils sont ouverts.
- Hors périmètre (non masqué) : Vue Élève (n'affiche que des prénoms), modales centrées sur UN élève ouvertes explicitement (fiche, historique), projection d'un bilan, exports et impressions (hors les deux blocages ci-dessus).

## 🪪 Fiche complète d'un élève (modale `mfiche`)

Clic droit sur une place → **🪪 Fiche complète** (`ctxOpenFiche` → `openFiche(sid)` → `renderFiche`), ou **clic sur le nom** dans l'onglet Élèves (`.stu-name-link`, `mousedown` stoppé pour ne pas déclencher la sélection multiple de la ligne). Tout ce que l'app sait de l'élève sur un écran, en **lecture seule**, pour un rendez-vous parents ou un conseil sans sauter d'onglet en onglet : identité et scolarité (classe, classes recomposées « aussi dans », groupe, tags, dates d'arrivée/départ, classes précédentes), aménagements (mêmes pastilles `.spec-*` via `_ficheSpecBadges`) et notes, place & matériel (salle active, n° QCM, tablette du pool actif, AESH liée), compteurs 📦/📝 et cumul appel toutes classes (détail par classe si plusieurs), rappels, et par discipline × période : moyenne /20 (`_computeStudentMeanForPeriod`), mentions de conseil, remarque bulletin. Les sections suivent les `featureFlags` (tablettes, appel, évaluations).

Les accès aux modales existantes sont posés **à côté de ce qu'ils concernent**, plus en boutons de pied (demande de l'utilisateur, 2026-09-25) : ✏️ après le nom dans le titre (Modifier) · 🕓 à côté des libellés **Compteurs** (historique, onglet Incidents), **Appel** (onglet Absences & retards) et du titre **Évaluations** (onglet Notes, toute l'année) ainsi que sur chaque ligne de période (onglet Notes filtré sur cette période) · clic sur la ligne **📋 Notes** (voir/éditer) · clic sur le titre **🔔 Rappels**. Classes `.fiche-ico` (icône discrète, agrandie en `pointer: coarse`) et `.fiche-click`. Tous passent par `_ficheGo(what, tab?, periode?)`, qui ferme la fiche, ouvre la modale et pose `_modalReturnTo[cible]` pour **rouvrir la fiche à la fermeture** — une parenthèse, pas une destination (même convention que ⚙ Motifs… / ⚙ Bibliothèque…). `openHist(sid, tab?, periode?)` accepte l'onglet et la période ; l'onglet **Notes** de l'historique a désormais des puces **Toute l'année / S1 / S2** (`_histNotesPeriode`, `setHistNotesPeriode`, filtre sur `ev.periode`, remis à « toute l'année » à chaque ouverture sans période). Le pied de la fiche ne garde que Fermer. Colonne des libellés en `width:1%` sinon le tableau la gonfle à la moitié de la largeur.
...
- **Tests (`test/`, `npm test`)** : harnais Node (`test/harness.js`) qui extrait le gros `<script>` inline, le charge dans un contexte `vm` avec un DOM stubé, neutralise `init()` et expose un pont `__TESTEVAL(code)` exécutant du code dans la portée lexicale du script (accès à `S` + toutes les fonctions hoistées). `test/run.test.js` (`node:test`) couvre la logique pure à risque : suppression en cascade (`_purgeStudentRefs`/`_deleteStudentInternal`/`deleteClass`), audit d'intégrité (`_auditState`), dates/périodes, parseurs (`_evalArithExpr`, `parseUnavailableInput`), validation d'import. **Aucune dépendance** (Node ≥ 18) ; le harnais n'est PAS livré avec l'app. Lancer `npm test` avant chaque push touchant cette logique.
  - **`test/compat.test.js` — rétrocompatibilité des fichiers JSON.** Pour chaque fixture de `test/fixtures/compat/`, rejoue le chemin d'un import (`_validateImport` → `S = data` → `postLoadHook` → `applyAccessorsAll`) et vérifie : aucune incohérence (inscrits/placés existants, salle active valide, champs pré-rooms et tablettes legacy disparus, types d'éval connus, discipline valide), audit d'intégrité vide, **`postLoadHook` idempotent** (second passage sans effet hors horloge), comptes conservés, état migré ré-importable. Les fixtures **historiques** sont générées par `node scripts/gen-compat-fixture.js <commit> <label>` : le script extrait le fichier au commit donné, rejoue **son** `createDemo()` dans le harnais et exporte `S` — c'est ce qu'un utilisateur de cette version aurait sauvegardé (v2026-05-14 sans évaluations · v2.0.0 · v2.12.2 · v2026-07-02 · v2026-08-02 avec 41 évals). `legacy-v1-pre-rooms.json` est écrit à la main au tout premier modèle (`cls.configuration` + `cls.seating` + `ipads*`, deux classes sur la même salle « B12 » → une seule salle au catalogue). ⚠️ **Ne pas régénérer une fixture existante** : elle fige un état historique, c'est sa valeur ; **ajouter une fixture à chaque changement de modèle** (nouvelle section de `S`, nouveau champ migré). Première prise : `createSalle` ne posait pas `ilots` / `collectPatterns` — une salle créée pendant `postLoadHook` (migration pré-rooms) ne les recevait qu'au chargement suivant. ⚠️ Le script vit dans `scripts/`, pas dans `test/` : `node --test` exécute **tout** `.js` sous `test/`, un utilitaire y passerait pour un test en échec. Les résultats sortis du `vm` transitent en **JSON** : un tableau d'un autre realm échoue à `deepStrictEqual` sur son prototype.
  - **`test/fuzz.test.js` — fichiers corrompus.** 150 mutations reproductibles (graine fixe) d'une fixture réelle : clés supprimées, valeurs absurdes (`null`, `'x'`, `[]`, `NaN`…), types changés ; le chemin d'import doit **refuser ou aboutir**, jamais lever. La première passe (300 mutations) a trouvé trois plantages à mi-migration — `S.eleves` absent, une entrée d'élève `null`, `cls.rooms = 'x'` — corrigés par **`_sanitizeCoreSections`** (appelée en tête de `postLoadHook` : sections `classes`/`eleves`/`salles` recréées si non-objet, entrées non-objet supprimées avec `console.warn`, `rooms` invalide réinitialisé) et par `_isPlainObj` dans `activeRoomData`. ⚠️ Une exception dans `postLoadHook` interrompt le chargement et laisse l'app sur un état à moitié migré, sans message : tout accès à une section de `S` dans une migration doit supposer qu'elle peut manquer ou être du mauvais type.
  - **`test/sync.test.js` — deux postes de bout en bout.** Deux sandboxes (= deux `localStorage`, deux `_DEVICE_ID`) échangent le JSON comme le fichier Nextcloud, et on vérifie la classification `_versionRelation` à chaque étape : `equal` → `ahead` (B écrit) → `diverged` (A écrit sans recharger) → résolution « garder ma version » (fusion max + bump) → B voit `ahead`, plus un Ctrl+Z chez A qui ne doit pas faire reculer l'horloge (B reste `behind`). Plus : poste vierge → `ahead` d'office ; horloge corrompue (chaînes, négatifs) fusionnée sans erreur — un compteur négatif n'est pas fusionné (clé absente = 0).
